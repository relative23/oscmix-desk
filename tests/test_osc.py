...
def test_mix_message_golden_bytes(session_mod):
    ...
    assert osc.encode_osc("/mix/5/playback/1", "fi", 0.0, -100) == expected
...
def test_message_length_is_multiple_of_four(session_mod):
    for path in ("/a", "/ab", "/abc", "/abcd", "/mix/10/playback/12"):
        assert len(osc.encode_osc(path, "fi", 1.5, 2)) % 4 == 0


def test_argument_count_mismatch_raises(session_mod):
    with pytest.raises(ValueError):
        osc.encode_osc("/x", "fi", 1.0)


def test_unsupported_type_tag_raises(session_mod):
    with pytest.raises(ValueError):
        osc.encode_osc("/x", "b", b"blob")


def test_decode_roundtrip(session_mod):
    message = osc.encode_osc("/mix/5/playback/1", "fi", -3.0, -100)
    path, tags, args = osc.decode_osc(message)
...
def test_decode_string_argument(session_mod):
    message = osc.encode_osc("/x", "s", "hello")
    assert osc.decode_osc(message) == ("/x", "s", ("hello",))


def test_decode_no_arguments(session_mod):
    assert osc.decode_osc(osc.encode_osc("/refresh")) == \
        ("/refresh", "", ())


def test_decode_rejects_malformed_input(session_mod):
    with pytest.raises(ValueError):
        osc.decode_osc(b"no-nul-terminator")
    with pytest.raises(ValueError):
        osc.decode_osc(b"/x\x00\x00garbage-tags\x00")
...
def test_decode_truncated_arguments_raise_value_error(session_mod):
    ...
    with pytest.raises(ValueError):
        osc.decode_osc(b"/x\x00\x00,f\x00\x00" + b"\x00\x00")
