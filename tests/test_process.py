...
def test_the_service_phase_is_the_status_line_or_empty(monkeypatch):
    from oscmix_desk import process

    seen = []
    monkeypatch.setattr(process, "_systemctl_output",
                        lambda *verb: seen.append(verb) or "verifying routing")
    assert process.service_phase() == "verifying routing"
    assert process.service_is_writing() is True
    assert seen[0] == ("show", "oscmix.service", "-p", "StatusText", "--value")
    assert len(seen) == 2, "is_writing asks the unit, it does not cache"
    for phase in ("running; verifier finished at 12:00:00", ""):
        monkeypatch.setattr(process, "_systemctl_output",
                            lambda *verb, phase=phase: phase)
        assert process.service_is_writing() is False


def test_systemctl_output_returns_stdout_only_on_success(monkeypatch,
                                                       real_systemctl_output):
    from oscmix_desk import process
