...
This inventory describes oscmix-desk 0.7.1 and the pinned oscmix revision
`f2fdd5ec78338848754aad32cc07f3440de63395`. Backend handlers and GTK bindings
were checked in that revision's source. A listed GTK control means a
binding exists; it is not a claim that every control has passed a physical
signal test. The [release evidence](evidence/0.7.1/) identifies the measured
subset and its firmware.
...
| Room EQ and crossfeed | Supported output settings and nine-band Room EQ are configurable. Delay remains in OSC units; physical delay and simultaneous DSP capacity are not established by register counts. | Backend handlers exist. No Room EQ or crossfeed controls are bound by the pinned GTK channel code. |
...
| DURec | No transport, file deletion or recorder-management CLI/config interface. | The backend has recorder commands and reports; GTK has transport/file controls. They are outside this release's hardware qualification. |
...
| Installation | 0.7.1 uses the source installer and a matching systemd user manager. Portable/manual installation and native packages are qualified separately on the installation branch. |
...
The runtime does not automatically select or refuse a live USB/rate mode.
...
also captures actual ALSA PCM. They do not qualify every physical connector,
external clock source, analog specification or Room EQ delay. Two physical
devices and other Fireface models remain unqualified.
